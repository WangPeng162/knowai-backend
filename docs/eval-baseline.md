...
2. ✅ chunk 切分升级（已完成：手写 800 硬切 → DocumentSplitters.recursive，见下）
3. rerank 与 embedding 分数 hybrid 融合（观察能否消除轻微退化题）
4. query rewriting（多轮指代消解）前后对比 → 观察指代类题
5. 数据集修正：Q8 关键词改为"两万多人"（或移除）——当前 @5=95% 上限的唯一原因
...
3. **线上 Top-2 命中率现在 = 95%**，离满分只差 Q8（数据缺失，库里无 19002）
4. 修掉 Q8 数据集后理论上 @2 可达 100%

### 全程进步轨迹（面试可讲的数据故事）

| 版本 | @1 | @2 | MRR |
|------|----|----|-----|
| v0 纯 embedding（旧切分） | 45% | 75% | 0.6742 |
| v1 + rerank | 60% | 90% | 0.7667 |
| v2 + 切分升级 | **80%** | **95%** | **0.8750** |
